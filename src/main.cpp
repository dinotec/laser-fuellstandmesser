...
const int FIFO_Anzahl = 30; // Anzahl der FIFO-Speicherplätze
...
void loop()
{
  ...
  { // phase failures have incorrect data
    ...
    FIFO[0] = measure.RangeMilliMeter; // Analogwert an erste Stelle des FIFO schreiben
    Analogwert = 0;
    ...
    Analogwert = (Analogwert / FIFO_Anzahl);

    tft.setTextSize(5);
    tft.setTextColor(TFT_GREEN);
    tft.drawString(String(Analogwert), tft.width() / 2, 30);
    tft.setTextColor(TFT_WHITE);
    tft.drawString("mm", tft.width() / 2, 70);

    tft.setTextColor(TFT_MAGENTA);
    tft.drawString(String(max(((570 - Analogwert) / 10), 0)), tft.width() / 2, 160);
    tft.setTextColor(TFT_WHITE);
    tft.drawString("cm", tft.width() / 2, 200);
  }
  ...
  else
  {
    tft.setTextSize(3);
    tft.setTextColor(TFT_RED);
    tft.drawString("OUT", tft.width() / 2, 70);
    tft.drawString("OF", tft.width() / 2, 130);
    tft.drawString("RANGE", tft.width() / 2, 190);
  }
